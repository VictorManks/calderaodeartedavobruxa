defmodule Calderaodeartesdavobruxa.Accounts do
  @moduledoc """
  ...
  alias Calderaodeartesdavobruxa.Accounts.{User, UserToken, UserNotifier}
  ...
  See `Calderaodeartesdavobruxa.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Calderaodeartesdavobruxa.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  @doc """
  Confirms a user account immediately.
  """
  ...
  def subscribe_opinions(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(Calderaodeartesdavobruxa.PubSub, "user:#{key}:opinions")
  end

  defp broadcast_opinion(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(Calderaodeartesdavobruxa.PubSub, "user:#{key}:opinions", message)
  end
